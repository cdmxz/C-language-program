...
void print_config(void)
{
	...
	char osVer[MAX_PATH], time[MAX_PATH], cpuInfo[MAX_PATH], card_info[MAX_PATH], userName[MAX_PATH];
	char disk[MAX_SIZE], InterFace[MAX_SIZE], boardInfo[MAX_SIZE];
	// 获取分辨率
	int w = GetSystemMetrics(SM_CXSCREEN);
	int h = GetSystemMetrics(SM_CYSCREEN);
	long long mem_size = GetPhysicalMemory();
	...
	GetDisplayCardInfo(card_info, sizeof(card_info));
...
void GetDisplayCardInfo(char* cardInfo, size_t strSize)
{
	//char info[MAX_PATH], data[MAX_PATH], time[MAX_PATH];
	...
	if (result == ERROR_SUCCESS)
	{
		...
		if (result == ERROR_SUCCESS && (p = strstr(info, "\\S")))
		{
