...
int OpenMusicList(const TCHAR* MusicList, TCHAR* MusicName, int* line, int mode)
{
	...
	{
		memset(C_FileName, 0, SIZE);// 数组清零
		fgets(C_FileName, SIZE, fp);// 从音乐列表中读取文件名
		Fgets_n(C_FileName);
		i++;// 统计读取了多少首歌	
	}
	...
	CharToTCHAR(C_FileName, MusicName, SIZE);// 将多字节转为宽字节
	return 0;
}
