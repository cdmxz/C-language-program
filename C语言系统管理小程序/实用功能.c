...
int multibyte_to_widebyte(char* sourceStr, wchar_t* destStr);// 多字节字符转宽字节字符
...
int enc_dec_file(char* source, char* dest)
{
    ...
    size_t len, j;
    char bar[102];
    long long currentMemory, fileSize, totalSize, i = 2;
    ...
    {
        for (j = 0; j <= len; j++)
            buffer[j] = ~buffer[j];//对读取到内存的内容进行异或加密
        ...
        // 打印进度
        per += ((double)len / (double)totalSize) * 100 + 0.5;
        per > max ? max : per;
        for (i = 0; i <= (unsigned)per / 2; i++)
            bar[i] = '>';
        bar[i] = '\0';
        printf("\r[%-50s] %.1lf%%", bar, per);
    }
...
int addstart(void)
{
    ...
    multibyte_to_widebyte(name, W_name);
    multibyte_to_widebyte(path, W_path);
...
int autoshut(void)
{
    ...
    else if (i == IDYES)
    {
        ...
        multibyte_to_widebyte(warning_two, str);
...
int multibyte_to_widebyte(char* sourceStr, wchar_t* destStr)
{
    wchar_t* buff = NULL;
